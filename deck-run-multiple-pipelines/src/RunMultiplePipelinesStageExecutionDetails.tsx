...
export function RunMultiplePipelinesStageExecutionDetails(props: IExecutionDetailsSectionProps) {
  ...
  return (
    <ExecutionDetailsSection name={props.name} current={props.current}>
        ...
        {props.stage.outputs.executionsList.length === 0 && executionsSet.size > 0 && (
          <>
            {Array.from(executionsSet).map((execution: any, index: any) => {
              return (
                <tr className="ng-scope info" analytics-on="click" analytics-category="Pipeline"
                    key={execution.id}>
                  ...
                  <td><span
                    className={"label label-default label-" + execution.status.toLowerCase()}>{execution.status}</span>
                  </td>
                  {execution.name != "rollbackOnFailure" &&
      ...
      {rollbackAllAppsModalOpen && <RollbackAllAppsModal setOpenModal={setRollbackAllAppsModalOpen}
